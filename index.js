...
selectTag.forEach((tag, id) => {
    for (const country_code in countries) {
        //selecting English by default to Russian
         let selected;
         ...
         tag.insertAdjacentHTML("beforeend", option);  //add option tag inside  select tag to get the list of countries
    }
});
...
icons.forEach(icon => {
    icon.addEventListener("click", ({target}) => {
        if (target.classList.contains("fa-copy")) {
            ...
            if (target.id == "from") {
                utterance = new SpeechSynthesisUtterance(fromText.value);
                utterance.lang = selectTag[0].value; //utterance setting in each selectTag
            } else {
                utterance = new SpeechSynthesisUtterance(toText.value);
                utterance.lang = selectTag[1].value;
            }
